...
| Static spawns (fishing nets, buckets) | query_nearby(object_name="X") | INTERACT_OBJECT <name> Take |
...
2. **GameObjects (Static Spawns)**: Permanent respawning items
   - Find: query_nearby(object_name="<item>")
   - Pick up: INTERACT_OBJECT <item_name> Take

### Example: Pick Up Fishing Net Spawn
```
Step 1: query_nearby(object_name="fishing net")
