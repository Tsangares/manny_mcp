...
| `on_failure` | string or dict | routine.py `_parse_on_failure` + the step-failure branch of `handle_execute_routine` | Per-step failure policy. **Backward-compatible default is `continue`** (absent/empty/`continue` = the legacy log-and-continue behavior — the runner records the error and proceeds to the next step). `abort` stops the whole run and records `abort_reason`/`aborted_at_step`. `retry:N` (or `{retry: N}`) re-sends the step up to N more times before falling back to `abort`. Unrecognized values fall back to `continue` (a typo can never silently abort). Note this is orthogonal to the inner-loop restart-on-failure mechanism, which only applies to steps inside a nested `inner` loop range. |
